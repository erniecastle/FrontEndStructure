...
var tipoCapturaPrin;
var nombreForma;
var editConsultaTama = ["mini-small", "mini-medium", "mini-large", "small-small", "small-medium", "small-large", "medium-small", "medium-medium", "medium-large", "large-small", "large-medium", "large-large", "extralarge-small", "extralarge-medium", "extralarge-large"];
...
function addListeners() {
    $(document).on('click', '.mainContent', function (e) {
        ...
        if (e.target.className.includes("mainPanelDivGroup") || e.target.className.includes("mainPanelContentComponents")
            ...
            /*|| e.target.id !== "headerEdit"*/ || e.target.className.includes("editKey")) {
    ...
    window.onkeydown = function (event) {
        if ($('#designer').children().length > 0) {
            if (event.key === "Escape") {
                backSelectToFather();
            }
            if (event.key === "Delete") {
                var idCmp = $(':focus')[0];
                if (idCmp !== undefined && idCmp.id !== undefined) {
                    if (!$('#' + idCmp.id).is('#panelPropertiesCmp *')) {
                        quitGlobalComponent();
                    }
                } else {
                    quitGlobalComponent();
...
function addContainerTabs() {
    ...
    if (divSelected === undefined ||
        ...
        var childTab = createChildTabs(tab, 1, container); // Buttons
...
function quitGlobalComponent() {
    ...
    if (confirm('¿Quieres eliminar este componente?')) {
        var divSelected = $('#content').find('.Selected')[0];
...
function createChildTabs(parent, identifier, container) {
    ...
    btn.id = 'btnTab' + (identifier) + (parent === null ? '' : 'F' + parent.id);
    if (parent.childNodes.length === 0) {
        btn.className = 'tablinks';
    } else {
        btn.className = 'tablinks borderleft';
    }
    btn.setAttribute('onclick', "openTab(event,'" + container.id + "')");
    var t = document.createTextNode('Tab ' + (identifier));
    btn.appendChild(t);
    return btn;
}
        ...
        if (divSelected.id.toString().startsWith('col')) {
            ...
            if (selectComponent !== '0') {
                ...
                if (selectComponent === 'LABEL') {
                    ...
                    if (propsComponent['capturaRango']) {
                ...
                else if (selectComponent === 'Edit' || ((configCaptura !== undefined && (configCaptura['tipoCaptura'] === "2" || configCaptura['tipoCaptura'] === "3" || configCaptura['tipoCaptura'] === "4")) && (propsComponent !== undefined || propsComponent['tipoDeDato'] === 0))) {
                    ...
                    addComp = createDiv(addComp, prop);;
...
function createComboBox(prop, tipoDato, tipoCaptura, totalelemen, titulo, selcomp) {
    ...
    if (selcomp) {
        if (selcomp === "Lista") {
            addComp.setAttribute('tipo', 'Lista');
        }
    }
    addComp.className = 'mainPanelContentComponents';
    addComp.style.width = prop.width;
...
function createTable(prop, numcol, numren) {
    ...
    if (existchecks > 0 || camposOrigenTabla.length > 0) {
        ...
        tableHeadEdit.id = "headerTable" + fuenteDatosPrincipal;
        //  var tableBodyEdit = document.createElement("TBODY");
        //  tableBodyEdit.id = "bodyEdit";
        addComp.appendChild(tableHeadEdit);
        // addComp.appendChild(tableBodyEdit);
        // Crea las celdas
        for (var i = 0; i < numren; i++) {
            ...
            tableHeadEdit.appendChild(encabezado);
...
function createEdit(prop, divSelected, tipoDato, tipoCaptura,config) {
    ...
    //labelEdit.innerHTML = divSelected.getAttribute("titulo");

    var divEdit = document.createElement('DIV');
    nameGlobal = 'E' + (numberCompInsideDiv + 1) + (divSelected === null ? '' : 'F' + divSelected.id);
    divEdit.id = 'edit' + config.origenes.origen;
    divEdit.className = 'edit';
   
    divEdit.setAttribute("functedit", "setEditObjectGeneric");
    divEdit.setAttribute("table", config.origenes.origen);
    divEdit.setAttribute("camposAmostrar", JSON.stringify([config.origenes.campovalor1, config.origenes.campovalor2]));
    if (divSelected.getAttribute("rango")) {
        divEdit.setAttribute("rango", divSelected.getAttribute("rango"));
        divEdit.setAttribute("dataedit", "Edit" + config.origenes.origen + divSelected.getAttribute("rango"));
        if (divSelected.getAttribute("rango") === "inicio") {
            labelEdit.innerHTML = "Del " + divSelected.getAttribute("titulo");
        } else {
            labelEdit.innerHTML ="Al "+ divSelected.getAttribute("titulo");
        }
    } else {
        divEdit.setAttribute("dataedit", "Edit" + config.origenes.origen);
        labelEdit.innerHTML = divSelected.getAttribute("titulo");
    }
...
function showPropertiesCom(comp) {
    ...
    if (onlyValid(comp)) {
        ...
        if (comp.tagName === 'LABEL') {
            ...
            if (!comp.className.includes("editKey")) {
                ...
                //addWidht(propComp); 
                //addLong(propComp);
                addWidthEditGen(propComp);
                addSelectComp(propComp);
                tipoCompoData = document.getElementById("tipoCompo");
                
                if (tipoCompoData) {
                    if (comp.getAttribute('tipoDato') === "0") {
                        ...
                        option5.text = "Edit Consulta";
                        option5.value = "Edit";
                        tipoCompoData.add(option5);

                    }
                    tipoCompoData.value = "Edit";
                }
            }
            addValorInicial(propComp, comp.id);
        }
            ...
            if (tipoCompoData) {
                ...
                if (comp.getAttribute('tipo')) {
                    tipoCompoData.value = comp.getAttribute('tipo');
                } else {
                    tipoCompoData.value = comp.type;
                }
            ...
            if (nombreForma !== 'fileCaptureSearch') {
...
function setSelectedColumns(cmpTable) {
    var dataTh = [];
    $("#" + cmpTable.id + ' >thead > tr > th').each(function () {
...
function updateProperties(comp, getProp) {
    ...
    if (document.getElementById('tipoCompo')) {
        ...
        if (componentSelected.getAttribute('type') !== null) {
            if (componentSelected.className.includes("editKey")) {
                typeOfElem = "Edit";
            } else {
        ...
        if ((typeOfElem === null || typeOfElem === undefined)) {
            if (componentSelected.getAttribute('tipo')) {
                typeOfElem = componentSelected.getAttribute('tipo');
            } else {
    ...
    if (componentIsChange) {
        if (onlyValid(componentSelected)) {
            ...
            if (document.getElementById("treeview")) {
                if (componentSelected.tagName === "TABLE") {
                    if (getProp) {
                        ...
                        if (document.getElementById('txtActivarAgregar')) {
            ...
            if (document.getElementById("editCompWay")) {
                if (getProp) {
                    var parent = componentSelected.parentNode;
                    if (parent.className.includes("mini-small")) {
                        document.getElementById("editCompWay").value = "mini-small";
                    } else if (parent.className.includes("mini-medium")) {
                        document.getElementById("editCompWay").value = "mini-medium";
                    } else if (parent.className.includes("mini-large")) {
                        document.getElementById("editCompWay").value = "mini-large";
                    } else if (parent.className.includes("small-small")) {
                        document.getElementById("editCompWay").value = "small-small";
                    } else if (parent.className.includes("small-medium")) {
                        document.getElementById("editCompWay").value = "small-medium";
                    } else if (parent.className.includes("small-large")) {
                        document.getElementById("editCompWay").value = "small-large";
                    } else if (parent.className.includes("medium-small")) {
                        document.getElementById("editCompWay").value = "medium-small";
                    } else if (parent.className.includes("medium-medium")) {
                        document.getElementById("editCompWay").value = "medium-medium";
                    } else if (parent.className.includes("medium-large")) {
                        document.getElementById("editCompWay").value = "medium-large";
                    } else if (parent.className.includes("large-small")) {
                        document.getElementById("editCompWay").value = "large-small";
                    } else if (parent.className.includes("large-medium")) {
                        document.getElementById("editCompWay").value = "large-medium";
                    } else if (parent.className.includes("large-large")) {
                        document.getElementById("editCompWay").value = "large-large";
                    } else if (parent.className.includes("extralarge-small")) {
                        document.getElementById("editCompWay").value = "extralarge-small";
                    } else if (parent.className.includes("extralarge-medium")) {
                        document.getElementById("editCompWay").value = "extralarge-medium";
                    } else if (parent.className.includes("extralarge-large")) {
                        document.getElementById("editCompWay").value = "extralarge-large";
                    }
                } else {
                    actualizarEditConsulta(componentSelected, document.getElementById("editCompWay").value);
                }
            }
...
function addWidthEditGen(propComp) {
    var labelWidht = document.createElement('LABEL');
    labelWidht.className = 'optionCFG';
    labelWidht.innerHTML = 'Tamaño Edit';
    labelWidht.setAttribute('style', 'display: block');
    propComp.appendChild(labelWidht);
    var cbDisplay = document.createElement('SELECT');
    cbDisplay.id = "editCompWay";
    cbDisplay.className = 'form-input small';
    propComp.appendChild(cbDisplay);
    for (var i = 0; i < editConsultaTama.length; i++) {
        var option = document.createElement("option");
        option.value = editConsultaTama[i];
        option.text = editConsultaTama[i];
        cbDisplay.add(option);
    }
}
    ...
    for (var i = 0; i < campos.length; i++) {
        ...
        if (isFather) {
            if (campos[i]['subOrigen']) {
                entity = campos[i]['subOrigen'].toString();
            ...
            if (level > 0) {
                if (campos[i]['origenNombre']) {
                    entity = campos[i]['origenNombre'].toString();
                    entity = entity.charAt(0).toLowerCase() + entity.slice(1);
                    input2.setAttribute('subEntity', entity);
                }
            }
        ...
        for (var i = 0; i < content.length; i++) {
            if (content[i].elemento === "ITHEAD") {
                ...
                for (var r = 0; r < colHeadCount; r++) {
                    if (content[i].content[r].elemento === "ITR") {
                        var colHead = document.createElement('TR');
                        if (typeof (content[i].attributes.id) !== 'undefined') {
                            colHead.setAttribute("id", content[i].content[r].attributes.id);
                        }
                        var count = content[i].content[r].content.length;
                        for (var j = 0; j < count; j++) {
                            if (content[i].content[r].content[j].elemento === "ITH") {
                                var th = document.createElement("TH");
                                if (typeof (content[i].content[r].content[j].attributes.id) !== 'undefined') {
                                    th.setAttribute("id", content[i].content[r].content[j].attributes.id);
                                }
                                if (typeof (content[i].content[r].content[j].attributes.fieldsource) !== 'undefined') {
                                    th.setAttribute("fieldsource", content[i].content[r].content[j].attributes.fieldsource);
                                }
                                if (typeof (content[i].content[r].content[j].attributes.idetiqueta) !== 'undefined') {
                                    th.setAttribute("idetiqueta", content[i].content[r].content[j].attributes.idetiqueta);
                                }
                                ...
                                var datacell = document.createTextNode(content[i].content[r].content[j].texto);
...
function elementSegmenter(cmp) {
    ...
    else if (cmp.tagName === "THEAD") {
        kindElement = "Contenedor Columnas";
    }
                    ...
                    if (element.attributes[i].nodeName === "style") {
                        ...
                        for (var p = 0; p < arrayStyle.length; p++) {
                            ...
                            if (arrayStyle[p] !== "") {//Add another estilos
                                ...
                                } else if (getPropertie[0]==="margin-bottom") {
                                    var valor = getPropertie[1].replace(/[12345]px/g, '');
                                    object["attributes"]["margenInferior"] = valor;
                                } else if (getPropertie[0]==="margin-top") {
                                    var valor = getPropertie[1].replace(/[12345]px/g, '');
                                    object["attributes"]["margenSuperior"] = getPropertie[1];
                                } else if (getPropertie[0]==="margin-left") {
                                    var valor = getPropertie[1].replace(/[12345]px/g, '');
                                    object["attributes"]["margenIzquierdo"] = valor;
                                } else if (getPropertie[0]==="margin-right") {
                                    var valor = getPropertie[1].replace(/[12345]px/g, '');
                                    object["attributes"]["margenDerecho"] = valor;
                                }
                            }
                        }
                    }
...
function getCamposOrigenDetalle(valId, behavior) {
    ...
    if (valId) {
        ...
        if (document.getElementById('cbxDetalle')) {
...
function createNuevoComponente(selcomp) {
    ...
    var res;
    if (typeAcces[1] !== "fileCapturaProceso") {
        res = getCampoOrigenPorID(divSelected.getAttribute("source"));
    } else {
        res = getParametrosProcesoPorID(divSelected.getAttribute("source"));
    }
   ...
   // console.log(valores);
    ...
    if (divSelected.getAttribute('tipocaptura') === "1") {
    ...
    } else if (divSelected.getAttribute('tipocaptura') === "2" || divSelected.getAttribute('tipocaptura') === "3" || divSelected.getAttribute('tipocaptura') === "4") {
        ...
        properties.width = divSelected.style.width;
        if (selcomp === "select-one") {
           
            if (divSelected.getAttribute('tipo')) {
                totalelemen = undefined;
            }
            if (document.getElementById('contRadio')) {
                ...
                if (divSelected.getAttribute('type') === 'radio') {
                    ...
                    if (divSelected.getAttribute('type')==="Edit") {
                        var parent2 = parent.parentNode.parentNode;
                        var label = document.createElement("label");
                        while (parent2.firstChild) {
                            if (parent2.firstChild.tagName === "LABEL") {
                                label.id = parent2.firstChild.id;
                            }
                    ...
                    addComp = createComboBox(properties, tipodato, tipocaptura, totalelemen, titulo);
                    //addComp.firstElementChild.style.maxWidth = "100%";
                    addComp.firstElementChild.style.width = "100%";
                    //addComp.setAttribute('titulo', titulo);
            ...
            if (document.getElementById('contRadio')) {
                ...
                if (divSelected.getAttribute('type') === 'radio') {
                    ...
                    // var titulo = divSelected.getAttribute('titulo');
                    parent.removeChild(divSelected);
                    addComp = createComboBox(properties, tipodato, tipocaptura, totalelemen, titulo, selcomp);
                    addComp.style.maxWidth = "100%";
                    addComp.style.width = "100%";
                   // addComp.setAttribute('size', totalelemen);
                    //addComp.setAttribute('titulo', titulo);
            ...
            if (document.getElementById('contRadio')) {
                ...
                if (divSelected.getAttribute('type') === 'radio') {
                    ...
                    // addComp = createComboBox(properties, tipodato, tipocaptura, totalelemen, titulo);
                    addComp = createEdit(properties, divSelected, tipodato, tipocaptura,config);
                   // addComp.style.maxWidth = "100%";
                   // addComp.style.width = "100%";
                    addComp.setAttribute('size', totalelemen);
                    //addComp.setAttribute('titulo', titulo);
                    parent2.appendChild(addComp);
                    if (typeof (parent.getAttribute('titulo') !== 'undefined')) {
                        parent.removeAttribute('titulo');
                    }
                }
...
function actualizarEditConsulta(componentSelected, value) {
    
    var parent = componentSelected.parentNode;
    parent.className = "edit " + value;
    var parentdiv = parent.parentNode.parentNode;
    parentdiv.style.maxWidth = "";
    var valores = value.split("-");
    var listCompo = parent.childNodes;
    for (var i = 0; i < listCompo.length; i++) {
        if (listCompo[i].className.includes("editKey")) {
            listCompo[i].className = "editKey " + valores[0]; 
        } else if (listCompo[i].className.includes("editBrief")) {
            listCompo[i].className = "editBrief " + valores[1]; 
        } else if (listCompo[i].className.includes("editButton")) {
            listCompo[i].className = "editButton small"; 
        }
    }
